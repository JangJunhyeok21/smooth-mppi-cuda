...
SATURATED_STEER_SAMPLE_WEIGHT=4.0
...
def main():
 p=argparse.ArgumentParser();p.add_argument('initial',nargs='?',default=str(INITIAL_MODEL_PATH));p.add_argument('--out',default=str(OUTPUT_PATH));p.add_argument('--epochs',type=int,default=EPOCHS);p.add_argument('--seed',type=int,default=SEED);a=p.parse_args();torch.manual_seed(a.seed);rng=np.random.default_rng(a.seed);dev=torch.device('cuda' if torch.cuda.is_available() else 'cpu');d=np.load(DATA);x=d['source_features'].astype(np.float32);obs=d['source_observations'].astype(np.float32);source_sr=d['source_speed_reference'].astype(np.float32) if 'source_speed_reference' in d.files else x[:,0].copy();b=d['source_bag_id'];sp=d['source_split'];v=d['source_valid'];tr=starts(x,b,sp,v,0);va=starts(x,b,sp,v,1);va=va[::max(1,len(va)//256)][:256];cfg=yaml.safe_load((ROOT/'config/params.yaml').read_text())['/**']['ros__parameters'];fit=json.loads(PARAMS.read_text())['expanded_fitted'];mean,std=load_norm(Path(a.initial)/'dynamic_40ms_residual.bin');net=Net();net.load_state_dict(torch.load(Path(a.initial)/'model.pt',map_location='cpu',weights_only=True));net.to(dev);X=torch.from_numpy(x).to(dev);OBS=torch.from_numpy(obs).to(dev);SOURCE_SR=torch.from_numpy(source_sr).to(dev);mean=torch.from_numpy(mean).to(dev);std=torch.from_numpy(std).to(dev);lf,lr,m,iz=[float(cfg[q]) for q in ('l_f','l_r','mass','dynamic_mlp_I_z')];wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;scale=float(cfg['kinematic_steer_scale']);bias=float(cfg['kinematic_steer_bias']);stau=float(cfg['steer_servo_time_constant']);srate=float(cfg['actuator_max_steer_rate']);kp=float(cfg['speed_servo_kp']);ata=float(cfg['speed_reference_accel_time_constant']);atb=float(cfg['speed_reference_brake_time_constant']);rrate=float(cfg['actuator_max_speed_reference_rate']);amin=float(cfg['min_accel']);amax=float(cfg['max_accel']);pscale=float(cfg['kinematic_position_speed_scale']);low=float(cfg['dynamic_mlp_min_speed']);res_yaw=float(cfg['dynamic_mlp_max_residual_yaw_accel']);gate_start=float(cfg['dynamic_mlp_residual_gate_steer_start']);gate_end=float(cfg['dynamic_mlp_residual_gate_steer_end']);total_yaw=float(cfg['dynamic_mlp_max_total_yaw_accel']);yaw_scale=float(cfg['dynamic_mlp_yaw_rate_kinematic_scale']);yaw_margin=float(cfg['dynamic_mlp_yaw_rate_margin']);yaw_ay=float(cfg['dynamic_mlp_yaw_rate_lateral_accel_limit'])
 def rollout(ids):
  ...
  for k in range(30):
   ...
   feat=torch.cat((current,c0,ap[:,None],(c0[:,0]-previous)[:,None],state,hist.reshape(len(ids),-1),acc),1);res=torch.clamp(net((feat-mean)/std),torch.tensor((-8.,-8.,-res_yaw),device=dev),torch.tensor((8.,8.,res_yaw),device=dev));lateral_gate=torch.sigmoid((torch.abs(current[:,0])-low)/.2);steer_gate=torch.clamp((gate_end-torch.abs(ap))/max(gate_end-gate_start,1e-4),0,1);ax_gate=(lateral_gate if GATE_AX_RESIDUAL else torch.ones_like(lateral_gate)) if USE_AX_RESIDUAL else torch.zeros_like(lateral_gate);gates=torch.stack((ax_gate,lateral_gate*steer_gate,lateral_gate*steer_gate),1);proposed=state+res*gates*.04;bounded_r=current[:,2]+torch.clamp(proposed[:,2]-current[:,2],-total_yaw*.04,total_yaw*.04);envelope=torch.minimum(yaw_scale*torch.abs(proposed[:,0]*torch.tan(ap)/wb)+yaw_margin,yaw_ay/torch.clamp(torch.abs(proposed[:,0]),min=.5));state=torch.stack((proposed[:,0],proposed[:,1],torch.clamp(bounded_r,-envelope,envelope)),1);acc=torch.stack((bax+res[:,0]*gates[:,0],ay+res[:,1]*gates[:,1]),1);yaw=pose[:,2];pose=torch.stack((pose[:,0]+pscale*(state[:,0]*torch.cos(yaw)-state[:,1]*torch.sin(yaw))*.04,pose[:,1]+pscale*(state[:,0]*torch.sin(yaw)+state[:,1]*torch.cos(yaw))*.04,yaw+state[:,2]*.04),1);resall.append(res)
 ...
 def loss(ids,return_per_window=False):
  ...
  value=(total+dense_yaw_rate+DENSE_VX_LOSS_WEIGHT*dense_vx
         ...
         +OVERSTEER_UNDERPREDICTION_WEIGHT*oversteer_under+1e-4*(res*res).mean())
