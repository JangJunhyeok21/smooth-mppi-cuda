...
int main(int argc,char **argv){
    ...
    p.dynamics_model=mppi::DYNAMIC_MLP_RESIDUAL_SERVO_LAG;p.min_accel=-1.f;p.max_accel=1.f;
    p.min_speed=std::atof(argv[12]);p.max_speed=std::atof(argv[13]);p.mass=3.74f;p.l_f=.163f;p.l_r=.161f;
    p.kinematic_steer_scale=.50927964f;p.kinematic_steer_bias=.01015773f;p.kinematic_position_speed_scale=.8633491306389823f;
    p.speed_servo_kp=.7616888694734905f;p.steer_servo_time_constant=.15514851356820727f;
    p.actuator_max_steer_rate=.8344090950084138f;p.speed_reference_accel_time_constant=.04f;
    ...
    p.dynamic_mlp_min_speed=.8f;const float wb=p.l_f+p.l_r;p.F_zf=p.mass*9.81f*p.l_r/wb;p.F_zr=p.mass*9.81f*p.l_f/wb;
    mppi::MPPISolver solver(1,std::max(2,steps),p);solver.load_dynamic_mlp_residual_weights(argv[1]);
    ...
    for(int k=0;k<steps;++k){
        ...
        s=solver.debug_dynamic_mlp_residual_step(s,u,h,true);
