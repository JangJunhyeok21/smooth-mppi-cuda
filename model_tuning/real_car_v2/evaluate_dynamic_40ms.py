...
def main():
 p=argparse.ArgumentParser();p.add_argument('result',nargs='?',default=str(RESULT_PATH));p.add_argument('--out',default=str(OUTPUT_PATH));p.add_argument('--data',default=str(DATA));p.add_argument('--classic-params',default=str(PARAMS));p.add_argument('--disable-mlp',action='store_true',default=DISABLE_MLP);p.add_argument('--bag-id',type=int,default=BAG_ID);a=p.parse_args();result=Path(a.result);w=load(result/'dynamic_40ms_residual.bin');d=np.load(a.data);x=d['source_features'].astype(float);b=d['source_bag_id'];sp=d['source_split'];v=d['source_valid'];cfg=yaml.safe_load((ROOT/'config/params.yaml').read_text())['/**']['ros__parameters'];fit=json.loads(Path(a.classic_params).read_text())['expanded_fitted'];c=Contract(dt=.04,steer_scale=float(cfg['kinematic_steer_scale']),steer_bias=float(cfg['kinematic_steer_bias']),steer_tau=float(cfg['steer_servo_time_constant']),max_steer_rate=float(cfg['actuator_max_steer_rate']),speed_kp=float(cfg['speed_servo_kp']),speed_accel_tau=float(cfg['speed_reference_accel_time_constant']),speed_brake_tau=float(cfg['speed_reference_brake_time_constant']),max_speed_reference_rate=float(cfg['actuator_max_speed_reference_rate']),position_speed_scale=float(cfg['kinematic_position_speed_scale']),min_accel=float(cfg['min_accel']),max_accel=float(cfg['max_accel']),low_speed_center=float(cfg['dynamic_mlp_min_speed']));lf,lr,m,iz=[float(cfg[q]) for q in ('l_f','l_r','mass','dynamic_mlp_I_z')];wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;H=30;report={}
