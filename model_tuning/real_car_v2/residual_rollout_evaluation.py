#!/usr/bin/env python3
"""Quantitatively evaluate held-out 1.2 s rollouts for unified Step 6."""
...
def stat(x):
 x=np.asarray(x,float);return {'rmse':float(np.sqrt(np.mean(x*x))),'mean':float(np.mean(x)),'median':float(np.median(x)),'p95':float(np.quantile(x,.95)),'max':float(np.max(x))}
def main():
 p=argparse.ArgumentParser();p.add_argument('result',nargs='?',default=str(RESULT_PATH));p.add_argument('--out',default=str(OUTPUT_PATH));p.add_argument('--data',default=str(DATA));p.add_argument('--classic-params',default=str(PARAMS));p.add_argument('--disable-mlp',action='store_true',default=DISABLE_MLP);p.add_argument('--disable-ax-residual',action='store_true');p.add_argument('--min-accel',type=float,help='평가에 사용할 종가속도 하한(미지정 시 YAML)');p.add_argument('--max-accel',type=float,help='평가에 사용할 종가속도 상한(미지정 시 YAML)');p.add_argument('--bag-id',type=int,default=BAG_ID);p.add_argument('--split-id',type=int);p.add_argument('--split-name',default='selected');a=p.parse_args();result=Path(a.result);w=load(result/'dynamic_40ms_residual.bin');input_dim=w[0].shape[1];d=np.load(a.data);x=d['source_features'].astype(float);obs=d['source_observations'].astype(float);source_sr=d['source_speed_reference'].astype(float) if 'source_speed_reference' in d.files else x[:,0];teacher_vy=x[:,1];b=d['source_bag_id'];sp=d['source_split'];v=d['source_valid'];cfg=yaml.safe_load((ROOT/'config/params.yaml').read_text())['/**']['ros__parameters'];mlp_cfg=yaml.safe_load((ROOT/'config/MLP_params.yaml').read_text())['/**']['ros__parameters'];fit=json.loads(Path(a.classic_params).read_text())['expanded_fitted'];min_accel=float(cfg['min_accel']) if a.min_accel is None else a.min_accel;max_accel=float(cfg['max_accel']) if a.max_accel is None else a.max_accel;c=Contract(dt=.04,steer_scale=float(cfg['kinematic_steer_scale']),steer_bias=float(cfg['kinematic_steer_bias']),steer_tau=float(cfg['steer_servo_time_constant']),max_steer_rate=float(cfg['actuator_max_steer_rate']),speed_kp=float(cfg['speed_servo_kp']),speed_accel_tau=float(cfg['speed_reference_accel_time_constant']),speed_brake_tau=float(cfg['speed_reference_brake_time_constant']),max_speed_reference_rate=float(cfg['actuator_max_speed_reference_rate']),position_speed_scale=float(cfg['kinematic_position_speed_scale']),min_accel=min_accel,max_accel=max_accel);lf,lr,m=[float(cfg[q]) for q in ('l_f','l_r','mass')];iz=float(fit.get('I_z',cfg['dynamic_mlp_I_z']));wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;H=30;report={'evaluation_contract':{'state_target':'causal classic-model KF','min_accel':min_accel,'max_accel':max_accel,'mlp_residual_limits':mlp_cfg}};res_limit=np.array((mlp_cfg['mlp_max_residual_ax'],mlp_cfg['mlp_max_residual_ay'],mlp_cfg['mlp_max_residual_yaw_accel']))
 ...
 for split,name in selected_splits:
  ...
  pa=np.asarray(predicted);ga=np.asarray(ground_truth);one=np.abs(pa[:,1]-ga[:,1]);one[:,2]=np.abs((pa[:,1,2]-ga[:,1,2]+np.pi)%(2*np.pi)-np.pi)
  horizon_metrics={}
  for step in (5,10,25):
   error=np.abs(pa[:,step]-ga[:,step]);error[:,2]=np.abs((pa[:,step,2]-ga[:,step,2]+np.pi)%(2*np.pi)-np.pi);horizon_metrics[f'{step*.04:.1f}s']={'trajectory_m':stat(np.linalg.norm(error[:,:2],axis=1)),'yaw_rad':stat(error[:,2]),'vx_mps':stat(error[:,3]),'vy_mps':stat(error[:,4]),'yaw_rate_rps':stat(error[:,5])}
  report[name]={'windows':len(starts),'one_step_40ms':{'trajectory_m':stat(np.linalg.norm(one[:,:2],axis=1)),'yaw_rad':stat(one[:,2]),'vx_mps':stat(one[:,3]),'vy_mps':stat(one[:,4]),'yaw_rate_rps':stat(one[:,5])},'short_horizon':horizon_metrics,'final_1p2s':{'trajectory_m':stat(pe),'yaw_rad':stat(ye),'vx_mps':stat(ve),'vy_mps':stat(vye),'yaw_rate_rps':stat(re)},'trajectory_m':stat(pe),'yaw_rad':stat(ye),'vx_mps':stat(ve),'vy_mps':stat(vye),'yaw_rate_rps':stat(re)}
