...
def python_rollout(cfg,fit,w):
 c=Contract(dt=.04,steer_scale=cfg['kinematic_steer_scale'],steer_bias=cfg['kinematic_steer_bias'],steer_tau=cfg['steer_servo_time_constant'],max_steer_rate=cfg['actuator_max_steer_rate'],speed_kp=cfg['speed_servo_kp'],speed_accel_tau=cfg['speed_reference_accel_time_constant'],speed_brake_tau=cfg['speed_reference_brake_time_constant'],max_speed_reference_rate=cfg['actuator_max_speed_reference_rate'],min_accel=cfg['min_accel'],max_accel=cfg['max_accel'])
 ...
 for k in range(STEPS):
  ...
  fyf=force(fzf,'f',af);fyr=force(fzr,'r',ar);ay=(fyf*np.cos(applied)+fyr)/m;rd=(lf*fyf*np.cos(applied)-lr*fyr)/iz;base=np.array([vx+(bax+vy*r)*.04,vy+(ay-vx*r)*.04,r+rd*.04]);feature=np.r_[state[3:6],steer,speed,applied,steer-previous,base,hist[:10],state[6:8]].astype(np.float32);res=np.clip(infer(feature,w),(0.,-8.,-30.),(0.,8.,30.));body=base+res*.04
...
def simulator_rollout(cfg,fit,binary):
 ...
 history=np.array([-.10,2.,-.05,2.2,0.,2.5,.08,2.8,.12,3.],np.float32);applied=np.float32(.07);speed_reference=np.float32(2.);rows=[]
  kwargs=dict(dt=.04,lf=cfg['l_f'],lr=cfg['l_r'],mass=cfg['mass'],min_speed=cfg['min_speed'],max_speed=cfg['max_speed'],min_accel=cfg['min_accel'],max_accel=cfg['max_accel'],speed_servo_kp=cfg['speed_servo_kp'],speed_accel_tau=cfg['speed_reference_accel_time_constant'],speed_brake_tau=cfg['speed_reference_brake_time_constant'],max_speed_reference_rate=cfg['actuator_max_speed_reference_rate'],steer_scale=cfg['kinematic_steer_scale'],steer_bias=cfg['kinematic_steer_bias'],steer_time_constant=cfg['steer_servo_time_constant'],max_steer_rate=cfg['actuator_max_steer_rate'],position_speed_scale=1.0,Bf=fit['B_f'],Cf=fit['C_f'],Df=fit['D_f'],Ef=fit['E_f'],Br=fit['B_r'],Cr=fit['C_r'],Dr=fit['D_r'],Er=fit['E_r'],Iz=cfg['dynamic_mlp_I_z'])
 for k in range(STEPS):
