...
enum class FsmState {
    ...
    OVERTAKE_PREP,  // 추월 준비 (좌/우 여유 공간 평가 중)
...
// 결정론적 방식(멀티모달 샘플링 없음): 방향은 h_pl/h_pr 클리어런스 계산만으로
// 결정하고, 실제 궤적 생성은 (a) 기존 장애물 회피 비용(obs_x/obs_y)과
// (b) 아래 speed_cap/바이패스 경로 스위칭만으로 이루어진다.
struct FsmCommand {
    FsmState state       {FsmState::SOLO};
    float    speed_cap   {5.0f};   // 이 틱에 적용할 mppi_params_.max_speed 상한
