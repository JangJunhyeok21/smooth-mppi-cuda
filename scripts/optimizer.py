...
class MPPIOptimizer(Node):
    def __init__(self):
        ...
        self.lat_g_threshold_list = [5.5, 6.5]
        self.lat_g_fault_threshold_list = [9.0, 10.0]
        ...
        # racecar_simulator의 car0 odom은 "/odom0"가 아니라 "/ekf_odom"으로 발행된다
        # (simulator.cpp). 예전엔 잘못된 토픽을 구독하고 있어 car_x/car_y가 갱신되지 않았다.
        ...
        # 실제 시뮬레이터 car0가 구독하는 토픽(/drive)으로 정지 명령을 보내야 한다.
    ...
    def start_mppi_node(self, q_v, q_dist, q_du, q_steer, q_lat_g, q_collision, boundary_margin,
        ...
        cmd = [
            ...
            "-p", f"lat_g_fault_threshold:={lat_g_fault_threshold}",
