...
def main():
 ...
 for i in range(len(s)):
  applied[i],_=actuator_step(applied[i-1] if i else applied[0],cmd[i,0],cmd[i,1],vx[i],c);sr,_=longitudinal_actuator_step(sr,cmd[i,1],vx[i],c);speedref[i]=sr
 ...
 for start in starts:
  ...
  for k in range(H):
   u=cmd[start+k];prev=hist[-1,0];delta,_=actuator_step(delta,u[0],u[1],state[0],c);sr,bax=longitudinal_actuator_step(sr,u[1],state[0],c);sv=max(abs(state[0]),.5);af=delta-np.arctan2(state[1]+lf*state[2],sv);ar=-np.arctan2(state[1]-lr*state[2],sv);fyf=fzf*Df*np.sin(Cf*np.arctan(Bf*af-Ef*(Bf*af-np.arctan(Bf*af))));fyr=fzr*Dr*np.sin(Cr*np.arctan(Br*ar-Er*(Br*ar-np.arctan(Br*ar))));bay=(fyf*np.cos(delta)+fyr)/m;brd=(lf*fyf*np.cos(delta)-lr*fyr)/iz;base=np.array([state[0]+(bax+state[1]*state[2])*dt,state[1]+(bay-state[0]*state[2])*dt,state[2]+brd*dt]);feat=np.r_[state,u,delta,u[0]-prev,base,hist.ravel()];res=net(feat,w)*low_speed_gate(state[0],c);old=state;state=base+res*dt;yaw=pose[2];pose=np.array([pose[0]+c.position_speed_scale*(state[0]*np.cos(yaw)-state[1]*np.sin(yaw))*dt,pose[1]+c.position_speed_scale*(state[0]*np.sin(yaw)+state[1]*np.cos(yaw))*dt,(yaw+state[2]*dt+np.pi)%(2*np.pi)-np.pi]);hist=np.vstack((hist[1:],u))
