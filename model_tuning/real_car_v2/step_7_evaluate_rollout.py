...
def main():
 p=argparse.ArgumentParser();p.add_argument('result',nargs='?',default=str(RESULT_PATH));p.add_argument('--out',default=str(OUTPUT_PATH));p.add_argument('--data',default=str(DATA));p.add_argument('--classic-params',default=str(PARAMS));p.add_argument('--disable-mlp',action='store_true',default=DISABLE_MLP);p.add_argument('--disable-ax-residual',action='store_true');p.add_argument('--bag-id',type=int,default=BAG_ID);p.add_argument('--split-id',type=int);p.add_argument('--split-name',default='selected');p.add_argument('--use-offline-vy-gt',action='store_true');a=p.parse_args();result=Path(a.result);w=load(result/'dynamic_40ms_residual.bin');input_dim=w[0].shape[1];d=np.load(a.data);x=d['source_features'].astype(float);obs=d['source_observations'].astype(float);source_sr=d['source_speed_reference'].astype(float) if 'source_speed_reference' in d.files else x[:,0];teacher_vy=d['source_teacher_vy'].astype(float) if a.use_offline_vy_gt else x[:,1];b=d['source_bag_id'];sp=d['source_split'];v=d['source_valid'];cfg=yaml.safe_load((ROOT/'config/params.yaml').read_text())['/**']['ros__parameters'];fit=json.loads(Path(a.classic_params).read_text())['expanded_fitted'];c=Contract(dt=.04,steer_scale=float(cfg['kinematic_steer_scale']),steer_bias=float(cfg['kinematic_steer_bias']),steer_tau=float(cfg['steer_servo_time_constant']),max_steer_rate=float(cfg['actuator_max_steer_rate']),speed_kp=float(cfg['speed_servo_kp']),speed_accel_tau=float(cfg['speed_reference_accel_time_constant']),speed_brake_tau=float(cfg['speed_reference_brake_time_constant']),max_speed_reference_rate=float(cfg['actuator_max_speed_reference_rate']),position_speed_scale=float(cfg['kinematic_position_speed_scale']),min_accel=float(cfg['min_accel']),max_accel=float(cfg['max_accel']),low_speed_center=float(cfg['dynamic_mlp_min_speed']));lf,lr,m,iz=[float(cfg[q]) for q in ('l_f','l_r','mass','dynamic_mlp_I_z')];wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;H=30;report={};res_yaw=float(cfg['dynamic_mlp_max_residual_yaw_accel']);gate_start=float(cfg['dynamic_mlp_residual_gate_steer_start']);gate_end=float(cfg['dynamic_mlp_residual_gate_steer_end']);total_yaw=float(cfg['dynamic_mlp_max_total_yaw_accel']);yaw_scale=float(cfg['dynamic_mlp_yaw_rate_kinematic_scale']);yaw_margin=float(cfg['dynamic_mlp_yaw_rate_margin']);yaw_ay=float(cfg['dynamic_mlp_yaw_rate_lateral_accel_limit'])
 ...
 for split,name in selected_splits:
  ...
  for start in starts:
   ...
   for k in range(H):
    ...
    feat=np.r_[current,c0,ap,c0[0]-previous,state,hist.ravel()];feat=np.r_[feat,acc] if input_dim==22 else feat;raw=np.zeros(3) if a.disable_mlp else net(feat,w);raw[2]=np.clip(raw[2],-res_yaw,res_yaw);gates=residual_gates(current[0],c);gates[0]=0. if a.disable_ax_residual else gates[0];steer_gate=np.clip((gate_end-abs(ap))/max(gate_end-gate_start,1e-4),0,1);gates[1:]*=steer_gate;res=raw*gates;proposed=state+res*.04;proposed[2]=current[2]+np.clip(proposed[2]-current[2],-total_yaw*.04,total_yaw*.04);envelope=min(yaw_scale*abs(proposed[0]*np.tan(ap)/wb)+yaw_margin,yaw_ay/max(abs(proposed[0]),.5));proposed[2]=np.clip(proposed[2],-envelope,envelope);state=proposed;acc=np.array((bax+res[0],ay+res[1]));yaw=pose[2];pose=np.array((pose[0]+c.position_speed_scale*(state[0]*np.cos(yaw)-state[1]*np.sin(yaw))*.04,pose[1]+c.position_speed_scale*(state[0]*np.sin(yaw)+state[1]*np.cos(yaw))*.04,yaw+state[2]*.04));trace.append(np.r_[pose,state])
