...
def main():
 d=np.load(SOURCE);x=d['features'].astype(float);derivative_targets=d['targets'].astype(float);teacher_vy=d['teacher_vy'].astype(float) if 'teacher_vy' in d.files else x[:,1];b=d['bag_id'];sp=d['split'];valid=d['valid'];source_dt=float(d['dt']);latency_steps=max(0,int(round(MPPI_COMPUTE_LATENCY_S/source_dt)));latency_quantized=latency_steps*source_dt
 if latency_quantized>=.04:raise ValueError('quantized MPPI compute latency must be shorter than model_dt=0.04 s')
 cfg=yaml.safe_load((ROOT/'config/params.yaml').read_text())['/**']['ros__parameters'];fit=json.loads(PARAMS.read_text())['expanded_fitted'];c=Contract(dt=.04,steer_scale=float(cfg['kinematic_steer_scale']),steer_bias=float(cfg['kinematic_steer_bias']),steer_tau=float(cfg['steer_servo_time_constant']),max_steer_rate=float(cfg['actuator_max_steer_rate']),speed_kp=float(cfg['speed_servo_kp']),speed_accel_tau=float(cfg['speed_reference_accel_time_constant']),speed_brake_tau=float(cfg['speed_reference_brake_time_constant']),max_speed_reference_rate=float(cfg['actuator_max_speed_reference_rate']),position_speed_scale=float(cfg['kinematic_position_speed_scale']),min_accel=float(cfg['min_accel']),max_accel=float(cfg['max_accel']),low_speed_center=float(cfg['dynamic_mlp_min_speed']));lf,lr,m,iz=[float(cfg[q]) for q in ('l_f','l_r','mass','dynamic_mlp_I_z')];wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;dt=.04;features=[];targets=[];bags=[];splits=[];oks=[];rows=[];command_rows=[]
 for i in range(len(x)-2):
  command_i=i+latency_steps
  ok=valid[i:i+3].all() and b[i]==b[i+1]==b[i+2] and sp[i]==sp[i+2]
  state=x[i,:3].copy();ap=float(x[i,5]);sr=float(state[0]);hist=x[command_i,10:20].copy();previous=x[max(i,command_i-1),3];cmd0=x[command_i,3:5]
  ...
  feature=np.r_[x[i,:3],cmd0,ap,cmd0[0]-previous,state,hist]
 ...
 payload=dict(features=np.asarray(features,np.float32),targets=np.asarray(targets,np.float32),teacher_confidence=(d['teacher_vy_confidence'][np.asarray(rows)+2].astype(np.float32) if 'teacher_vy_confidence' in d.files else np.ones(len(rows),np.float32)),bag_id=np.asarray(bags),split=np.asarray(splits),valid=np.asarray(oks),source_row=np.asarray(rows),command_source_row=np.asarray(command_rows),source_features=x.astype(np.float32),source_teacher_vy=teacher_vy.astype(np.float32),source_teacher_vy_confidence=(d['teacher_vy_confidence'].astype(np.float32) if 'teacher_vy_confidence' in d.files else np.ones(len(x),np.float32)),source_bag_id=b,source_split=sp,source_valid=valid,feature_names=np.array(FEATURES),target_names=np.array(OUTPUTS),control_dt=source_dt,model_dt=.04,mppi_compute_latency_requested_s=MPPI_COMPUTE_LATENCY_S,mppi_compute_latency_quantized_s=latency_quantized,mppi_compute_latency_range_s=np.asarray(MPPI_COMPUTE_LATENCY_RANGE_S),command_response_time_in_first_knot_s=.04-latency_quantized,alignment_contract="state_at_subscribe_time_command_at_publish_time",vy_input_contract=input_contract,vy_teacher_contract="offline_smoother_with_causal_fallback")
