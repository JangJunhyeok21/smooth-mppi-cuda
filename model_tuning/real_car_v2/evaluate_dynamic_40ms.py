...
def main():
 p=argparse.ArgumentParser();p.add_argument('result',nargs='?',default=str(RESULT_PATH));p.add_argument('--out',default=str(OUTPUT_PATH));p.add_argument('--disable-mlp',action='store_true',default=DISABLE_MLP);p.add_argument('--bag-id',type=int,default=BAG_ID);a=p.parse_args();result=Path(a.result);w=load(result/'dynamic_40ms_residual.bin');d=np.load(DATA);x=d['source_features'].astype(float);b=d['source_bag_id'];sp=d['source_split'];v=d['source_valid'];cfg=yaml.safe_load((ROOT/'config/params.yaml').read_text())['/**']['ros__parameters'];fit=json.loads(PARAMS.read_text())['expanded_fitted'];c=Contract(steer_scale=float(cfg['kinematic_steer_scale']),steer_bias=float(cfg['kinematic_steer_bias']),steer_tau=float(cfg['steer_servo_time_constant']),max_steer_rate=float(cfg['actuator_max_steer_rate']),speed_kp=float(cfg['speed_servo_kp']),speed_accel_tau=float(cfg['speed_reference_accel_time_constant']),speed_brake_tau=float(cfg['speed_reference_brake_time_constant']),max_speed_reference_rate=float(cfg['actuator_max_speed_reference_rate']),position_speed_scale=float(cfg['kinematic_position_speed_scale']),min_accel=float(cfg['min_accel']),max_accel=float(cfg['max_accel']),low_speed_center=float(cfg['dynamic_mlp_min_speed']));lf,lr,m,iz=[float(cfg[q]) for q in ('l_f','l_r','mass','dynamic_mlp_I_z')];wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;H=30;report={}
 selected_splits=((int(sp[np.flatnonzero(b==a.bag_id)[0]]),f'bag_{a.bag_id}') ,) if a.bag_id is not None else ((1,'validation'),(2,'test_aggressive'))
 for split,name in selected_splits:
  ...
  for start in starts:
   state=x[start,:3].copy();ap=float(x[start,5]);sr=float(state[0]);hist=x[start,10:20].reshape(5,2).copy();pose=np.zeros(3);trace=[np.r_[pose,state]]
   for k in range(H):
    i=start+2*k;c0=x[i,3:5];c1=x[i+1,3:5]
    if k>0:hist=np.vstack((hist[1:],c0))
    previous=hist[-2,0];current=state.copy()
    for cmd in (c0,c1):
     ap,_=actuator_step(ap,cmd[0],cmd[1],state[0],c);sr,bax=longitudinal_actuator_step(sr,cmd[1],np.hypot(state[0],state[1]),c);vx,vy,r=state;safe=max(abs(vx),.5);af=ap-np.arctan2(vy+lf*r,safe);ar=-np.arctan2(vy-lr*r,safe);bf=fit['B_f']*af;br=fit['B_r']*ar;fyf=fzf*fit['D_f']*np.sin(fit['C_f']*np.arctan(bf));fyr=fzr*fit['D_r']*np.sin(fit['C_r']*np.arctan(br));ay=(fyf*np.cos(ap)+fyr)/m;rd=(lf*fyf*np.cos(ap)-lr*fyr)/iz;state=np.array((vx+(bax+vy*r)*.02,vy+(ay-vx*r)*.02,r+rd*.02))
    feat=np.r_[current,c0,ap,c0[0]-previous,state,hist.ravel()];res=(np.zeros(3) if a.disable_mlp else net(feat,w)*low_speed_gate(current[0],c));state=state+res*.04;yaw=pose[2];pose=np.array((pose[0]+c.position_speed_scale*(state[0]*np.cos(yaw)-state[1]*np.sin(yaw))*.04,pose[1]+c.position_speed_scale*(state[0]*np.sin(yaw)+state[1]*np.cos(yaw))*.04,yaw+state[2]*.04));hist=np.vstack((hist[1:],c1));trace.append(np.r_[pose,state])
