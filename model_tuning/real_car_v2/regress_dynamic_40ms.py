...
def rollout(p,d,st,cfg):
 x=d['features'];n=len(st);state=x[st,:3].astype(float).copy();ap=x[st,5].astype(float).copy();sr=state[:,0].copy();pred=[];gt=[];Bf,Cf,Df,Ef,Br,Cr,Dr,Er=expand(p);lf,lr,m,iz=[float(cfg[q]) for q in ('l_f','l_r','mass','dynamic_mlp_I_z')];wb=lf+lr;fzf=m*9.81*lr/wb;fzr=m*9.81*lf/wb;dt=.02
 for k in range(H):
  for sub in range(2):
   row=st+2*k+sub;cmd=x[row,3:5];target=np.clip(float(cfg['kinematic_steer_scale'])*cmd[:,0]+float(cfg['kinematic_steer_bias']),-.55,.55);rate=np.clip((target-ap)/float(cfg['steer_servo_time_constant']),-float(cfg['actuator_max_steer_rate']),float(cfg['actuator_max_steer_rate']));ap=np.clip(ap+rate*dt,-.55,.55);speed=np.clip(cmd[:,1],float(cfg['min_speed']),4.);tau=np.where(speed>=sr,float(cfg['speed_reference_accel_time_constant']),float(cfg['speed_reference_brake_time_constant']));sr+=np.clip((speed-sr)/tau,-float(cfg['actuator_max_speed_reference_rate']),float(cfg['actuator_max_speed_reference_rate']))*dt;vx,vy,r=state.T;ax=np.clip(float(cfg['speed_servo_kp'])*(sr-np.hypot(vx,vy)),float(cfg['min_accel']),float(cfg['max_accel']));safe=np.maximum(abs(vx),.5);af=ap-np.arctan2(vy+lf*r,safe);ar=-np.arctan2(vy-lr*r,safe);bf=Bf*af;br=Br*ar;fyf=fzf*Df*np.sin(Cf*np.arctan(bf-Ef*(bf-np.arctan(bf))));fyr=fzr*Dr*np.sin(Cr*np.arctan(br-Er*(br-np.arctan(br))));ay=(fyf*np.cos(ap)+fyr)/m;rd=(lf*fyf*np.cos(ap)-lr*fyr)/iz;state=np.c_[vx+(ax+vy*r)*dt,vy+(ay-vx*r)*dt,r+rd*dt]
  pred.append(state.copy());gt.append(x[st+2*(k+1),:3])
...
def residual(p,d,st,cfg,reg=True):
 pr,gt=rollout(p,d,st,cfg);w=np.linspace(.25,1,H)[None,:,None];z=((pr-gt)*np.array((.4,2.,1.5))[None,None,:]*w).ravel();pp=rel(pr,float(cfg['kinematic_position_speed_scale']));gp=rel(gt,float(cfg['kinematic_position_speed_scale']));z=np.r_[z,2*(pp[:,-1,:2]-gp[:,-1,:2]).ravel()]
